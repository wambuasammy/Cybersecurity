...
We create a Dockerfile that defines how the image will be built.
...
We build a Docker image using the Dockerfile.
...
We run a container from the previously built image.
...
- `my-python-app` is the image built in the previous step  
...
We start a container in interactive mode to access its internal environment.
...
This allows direct interaction with the container’s environment.
...
This step allows interaction with the container's runtime layer, not the image itself.

All changes made here exist only inside this container.
...
![Modified Container](screenshots/step7_modify.png)
...
- The file `newfile.txt` exists in the first container  
...
- Images provide a consistent and reproducible environment  
- Containers provide isolated and temporary execution  
